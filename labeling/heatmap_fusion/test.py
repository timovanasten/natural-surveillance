...
from common_pytorch.net_modules import valid_net, eval_net
...
def main():
    ...
    logger.debug("Using Devices: {}".format(str(devices)))
    ...
    dataset_valid = FacadeDataset(valid_imdbs, False, config.train.patch_width, config.train.patch_height,
    ...
    heatmaps, tagmaps, vloss = \
        valid_net(valid_data_loader, net, loss_func, merge_hm_flip_func, merge_tag_flip_func,
                  devices, flip_pairs, flip_test=True)
    endt1 = time.time() - beginT
    logger.info('Valid Loss:%.4f' % vloss)

    beginT = time.time()
    eval_net(0, heatmaps, tagmaps, valid_data_loader, config.loss, config.test,
