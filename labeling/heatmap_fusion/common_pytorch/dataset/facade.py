...
def calc_pixel_acc(windows, winBoolMap, totalGTPixel):
    '''
    ...
    def parse_json_annotation(self, annotation_path, im_path, shape):
        ...
        for item in label['shapes']:
            if item['label'] == 'window':
                ...
                assert is_valid(shape, left_top) and is_valid(shape, left_bottom) and \
                       is_valid(shape, right_bottom) and is_valid(shape, right_top), \
        ...
        window_list = sorted(window_list, key=cmp_to_key(sort_window))
        return window_list
