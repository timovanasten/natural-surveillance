...
def train_net(nth_epoch, train_data_loader, network, optimizer, loss_config, loss_func, speedometer=None):
    """
    :param nth_epoch:
...
def valid_net(valid_data_loader, network, loss_func, merge_hm_flip_func, merge_tag_flip_func,
...
def eval_net(nth_epoch, heatmaps, tagmaps, valid_data_loader, loss_config, test_config,
...
def infer_net(infer_data_loader, network, merge_hm_flip_func, merge_tag_flip_func, flip_pairs,
              patch_width, patch_height, loss_config, test_config, final_output_path, flip_test=True):
    """
    Infers the window locations in the image
    ...
    for n_s in range(num_samples):
        try:
            ...
            # No windows detected, append empty list
            windows_list_with_score.append([])
            continue
