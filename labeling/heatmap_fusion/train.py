...
from common_pytorch.net_modules import train_net, valid_net, eval_net

from common_pytorch.blocks.resnet_pose import get_default_network_config, init_pose_net, get_pose_net
from common_pytorch.loss.heatmap import get_default_loss_config, get_loss_func, get_label_func, get_merge_func

from core.loader import FacadeDataset
...
def main():
    ...
    dataset_train = FacadeDataset(train_imdbs, True, config.train.patch_width, config.train.patch_height,
                                  label_func, config.aug, config.loss)

    dataset_test = FacadeDataset(test_imdbs, False, config.train.patch_width, config.train.patch_height,
                                 label_func, config.aug, config.loss)
    ...
    for epoch in range(config.train.begin_epoch, config.train.end_epoch + 1):
        ...
        beginT = time.time()
        tloss = train_net(epoch, train_data_loader, net, optimizer, config.loss, loss_func, speedometer)
        endt1 = time.time() - beginT

        beginT = time.time()
        heatmaps, tagmaps, vloss = valid_net(valid_data_loader, net, loss_func, merge_hm_flip_func,
        ...
        if epoch > config.train.end_epoch - 3: #only eval late model, because evaluation takes too much time
            eval_net(epoch, heatmaps, tagmaps, valid_data_loader, config.loss, config.test,
