...
def debug_vis(img, window_corner, label=None, raw_img=None, plotLine=True):
    if isinstance(img, str):
        ...
        assert 0, "unKnown Type of visualization in debug_vis"
...
def vis_eval_result(img, window, plot_line=False, save_filename=None):
    if isinstance(img, str):
        if os.path.exists(img):
            cv_img_patch_show = cv2.imread(img)
    elif isinstance(img, np.ndarray):
    ...
    if save_filename != None:
        ...
        if not os.path.exists(dirname):
            os.makedirs(dirname)
        cv2.imwrite(os.path.join(save_filename), cv_img_patch_show, [cv2.IMWRITE_PNG_COMPRESSION, 9])
    else:
